...
For JPG, PNG, WebP, and BMP images, choose **Original**, **JPEG (photos)**, or **PNG (text clarity)** in the preview dialog. JPEG uses adjustable quality and turns transparent areas white; PNG uses lossless encoding for sharp text and may create a larger file. Set a maximum width in pixels, or 0 to keep the original width. The height follows the aspect ratio and images are never enlarged. Reducing width can still make text too small to read. The preview shows the output dimensions and file size. **Cancel** leaves the file unchanged; saving replaces its image data, updates the filename and Obsidian links, and gives it a numbered filename if needed to avoid Obsidian's old image cache. GIF and SVG files remain rename-only.

To process an image already in the vault, right-click its file in Obsidian and choose **Rename or convert image…**. In plugin settings, choose whether pasted images start with JPEG conversion or lossless PNG text clarity selected. Text clarity takes priority if both defaults are enabled. You can also set the starting JPEG quality. The preview dialog appears whenever a processing default is active, even with **Auto rename** enabled.
